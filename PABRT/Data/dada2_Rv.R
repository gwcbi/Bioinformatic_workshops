...
path <- "/Users/marcosperez-losada/Dropbox/Projects/UPR-Ponce/UPR-Ponce_FASTQ/fastq" 
list.files(path)
...
out <- filterAndTrim(fnRs, filtRs, trimLeft=0, trimRight=0, truncLen=c(145), # adjsut as needed
...
track <- cbind(out, sapply(dadaRs, getN), rowSums(seqtab.nochim))

# If processing a single sample, remove the sapply calls: e.g. replace sapply(dadaFs, getN) with getN(dadaFs)
colnames(track) <- c("input", "filtered", "denoisedR", "nonchim")
...
taxa1 <- assignTaxonomy(seqtab.nochim, "/Users/marcosperez-losada/Software/dada2/silva_nr99_v138_wSpecies_train_set.fa.gz", multithread=TRUE)
taxa1->taxa
...
write.table(taxa3,"dada2_Rv_taxonomy.txt",quote=F,sep="\t")
...
export_taxa_table_and_seqs(seqtab.nochim,"ASV_Rv_table.txt","ASV_Rv_seqs.fa")
...
# Option 2: launch QIIME2 and execute: 
  # qiime tools import --type 'FeatureData[Sequence]' --input-path dada2/ASV_seqs.fa --output-path ASV_seqs.qza
  # qiime phylogeny align-to-tree-mafft-fasttree --i-sequences ASV_seqs.qza --output-dir mafft-fasttree-output-dada2
  # qiime tools export --input-path mafft-fasttree-output-dada2/tree.qza  --output-path dada2
  # qiime tools export --input-path mafft-fasttree-output-dada2/masked_alignment.qza  --output-path masked_alignment.fa
## Create phyloseq object using dada2 tables and trees generated above
#  Tables have to be adjusted

setwd("/Users/marcos/Dropbox/Projects/UPR-Ponce/course")

tree <- read_tree("ASV_Rv_tree.txt")
taxa1 <- read.delim("dada2_Rv_taxonomy_R.txt")
otu <- read.delim("ASV_Rv_table.txt", skip = 1, row.names = 1, check.names = FALSE)
metadata <- read.csv("ASV_metadata_03.02.2021.csv", header=TRUE, row.names = 1)
...
# keep bacteria and eliminate unclassified reads at the Phylum level
# Silva
ps2<-subset_taxa(ps1, Kingdom=="Bacteria" & Phylum!="unclassified")
ps2
...
write.csv(table_otu,file="ASV_Rv_table.csv")

#Export taxonomy of ASVs
table_tax<-tax_table(ps5)
write.csv(table_tax,file="dada2_Rv_taxonomy_v1.csv")
