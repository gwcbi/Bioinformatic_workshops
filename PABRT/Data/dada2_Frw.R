...
path <- "/Users/marcos/Dropbox/Projects/UPR-Ponce/UPR-Ponce_FASTQ/fastq" 
list.files(path)
...
out <- filterAndTrim(fnFs, filtFs, trimLeft=0, trimRight=0, truncLen=c(145), # adjsut as needed
...
write.table(taxa3,"dada2_Frw_taxonomy.txt",quote=F,sep="\t")
...
export_taxa_table_and_seqs(seqtab.nochim,"ASV_Frw_table.txt","ASV_Frw_seqs.fa")
...
taxa1 <- read.delim("dada2_Frw_taxonomy_R.txt")
otu <- read.delim("ASV_Frw_table.txt", skip = 1, row.names = 1, check.names = FALSE)
metadata <- read.csv("ASV_metadata_R.csv", header=TRUE, row.names = 1)
...
write.csv(table_otu,file="silva_table_ps5.csv")

#Export taxonomy of ASVs
table_tax<-tax_table(ps5)
write.csv(table_tax,file="silva_tax_ps5.csv")
